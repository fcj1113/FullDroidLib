package cn.fulldroid.lib.statusbar;
...
public class StatusBarHelper {
    ...
    public static void setStatusBarDark(Activity activity){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isMIUI6Later() || isFlyme4Later() || Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                setStatusBarDarkMode(activity);
                tintStatusBar(activity, Color.parseColor("#eaeaea"), 0);
            } else {
                tintStatusBar(activity, Color.parseColor("#bbbbbb"), 0);
            }
        }
    }
